// Vendored verbatim from ../../../omarchy/shell/Commons/Border.qml. Its
// Color.shellValues lookups (see value()/valueOr() below) always return ""
// here since our Color.qml (adapted, see its own header comment) never
// populates shellValues - border style overrides from a theme's shell.toml
// are simply inert, every caller falls through to its own fallback. See
// Style.qml's header comment in this same directory.
pragma Singleton
import QtQuick
