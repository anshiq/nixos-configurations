// Vendored verbatim from ../../../omarchy/shell/Commons/Style.qml (Omarchy
// "Quattro" reference checkout) so real Omarchy bar-widget plugins that
// `import qs.Commons` resolve Style.* without modification - see
// quickshell/scripts/plugin.sh's header comment and ../../plugins/
// default.nix's ianswope.plex entry for the plugin this exists for. Only
// Color.qml in this directory was adapted (its own file has the diff);
// this file is unmodified from upstream and self-contained (hyprctl/
// fc-match are generic tools present on this system too, and its two
// Omarchy-state FileViews at the bottom just no-op here via
// printErrors: false).
pragma Singleton
import QtQuick
import Quickshell
